...
public func inInterval<T:Comparable>(_ expectedInterval: Range<T>) -> Matcher<T> {
    return Matcher("in interval \(expectedInterval)") {expectedInterval.contains($0)}
}

public func inInterval<T:Comparable>(_ expectedInterval: ClosedRange<T>) -> Matcher<T> {
  return Matcher("in interval \(expectedInterval)") {expectedInterval.contains($0)}
}
